 ...
 * @return {Boolean}
 */
export const isPhoneNumber = (phone: string | number): boolean => {
...
// 本地存储
export const setStorage = (key: string, value: any, _expireDate?: expireDateType) => {
      // 添加过期时间
      if (_expireDate) {
            const { key, value } = _expireDate
            const expireDate = moment().add(value, key).format("YYYY-MM-DD HH:mm:ss")
...
export const getStorage = (key: string) => {
      ...
      if (json.expireDate) {
            if (moment().isAfter(json.expireDate)) {
                  removeStorage(key)
                  return null
            }
            return json.value
 ...
 * @returns {Promise} Promise
 */
export const confirm = (msg: string, options?: optionsType) => {
      return wx.showModal(
            Object.assign(
                  {
                        title: "提示",
 ...
 * @returns Promise
 */
export const toast = (title: string, icon: "success" | "error" | "none" | "loading" = "none", duration: number = 1500) => {
...
export const systemInfo = () => {
      ...
      setStorage("systemInfo", systemInfo)
      ...
      setStorage("statusHeight", statusHeight * pixelRatio)

      //导航栏高度
      let navHeight = (menu.top - statusHeight) * 2 + menu.height
      setStorage("navHeight", navHeight * pixelRatio)

      //顶部高度 = 状态栏高度 + 导航栏高度
      setStorage("topHeight", (statusHeight + navHeight) * pixelRatio)
}
